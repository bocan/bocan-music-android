...
4. `SyncForegroundService`: started for active transfers, `dataSync` foreground service type, progress notification (indeterminate during manifest, determinate during transfer, with a Cancel action). Stops itself when the engine returns to Idle/Done. Platform and store constraints (binding):
   - Android 15+ caps `dataSync` services at 6 hours of runtime; the engine's resumability makes this a non-event, but the service must handle `onTimeout()` by pausing gracefully rather than crashing.
   - Never start this service from a `BOOT_COMPLETED` receiver (disallowed on Android 15+ for `dataSync`); triggers are discovery, user action, and WorkManager only.
   - Google Play requires a Console declaration for the `dataSync` type, including a short video showing the user triggering a sync. Phase 12's release checklist owns filing it; this phase owns making the flow trivially demonstrable (Settings -> Sync Now).
5. `SyncTriggers`:
