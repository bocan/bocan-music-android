...
6. **Privacy and store metadata**: data-safety declaration (no data collected; optional scrobble providers send listening data to the user's chosen service; sync stays on-LAN), permissions rationale (network, foreground service dataSync + mediaPlayback, POST_NOTIFICATIONS), store listing draft in `store/` with screenshots generated from the app in both themes.
7. **Release checklist** (`docs/release-checklist.md`): the phase 10 manual matrix (Auto DHU, Bluetooth, widget), a pairing + full-sync run against a real Mac, upgrade-in-place test from the previous release (database and DataStore migrations exercised), and the R8 smoke.
