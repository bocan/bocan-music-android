...
1. Dependencies: `media3-exoplayer`, `media3-session`, `media3-common`, plus `media3-decoder-ffmpeg`. The FFmpeg extension is not published on Maven by Google; decide between (a) the Jellyfin-published prebuilt (`org.jellyfin.media3:media3-ffmpeg-decoder`, tracks upstream Media3 versions) and (b) building the extension from the androidx/media source with an NDK toolchain. Default to (a) for v1 with the decision recorded in the PR; pin its version to match the Media3 version exactly.
   **16 KB page-size requirement (Play-blocking)**: these `.so` files are the app's only native code, and Google Play requires 16 KB ELF alignment for apps targeting Android 15+. If using the prebuilt, verify alignment before adopting it (`check_elf_alignment.sh` from the Android docs, or apkanalyzer on the packaged APK) and record the result in the PR; a misaligned artifact forces route (b). If building from source, use NDK r28+ (aligned by default) and AGP 8.5.1+. Add an alignment check to CI so a future artifact bump cannot silently regress it.
